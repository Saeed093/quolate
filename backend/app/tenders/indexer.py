...
async def index_tender(session: AsyncSession, payload: dict) -> None:
    ...
    await session.execute(
        delete(TenderEmbedding).where(TenderEmbedding.tender_id == tender.id)
    )

    # 1. Detail-page / notice text.
    header = "\n".join(
        p for p in [tender.title, tender.organization, tender.tender_no] if p
    )
    body = tender.raw_text or ""
    try:
        n = await _embed_chunks(
            session, tender, f"{header}\n{body}".strip(), tender_document_id=None
        )
        log.info("indexed tender %s text: %d chunks", tender.tender_no, n)
    except Exception:
        log.warning("tender %s text indexing failed", tender.tender_no, exc_info=True)
    ...
    if not tender.detail_url:
        return
    try:
        html = await asyncio.to_thread(_fetch_text, tender.detail_url)
        links = _find_document_links(html, tender.detail_url)
    except Exception:
        log.warning(
            "tender %s detail page fetch failed", tender.tender_no, exc_info=True
        )
        return
    ...
    for url, label in links:
        try:
            ...
            storage_key = f"tenders/{tender.id}/documents/{sha}{ext}"
            ...
            n = await _embed_chunks(
                session, tender, content.full_text, tender_document_id=doc.id
            )
            doc.status = "parsed"
            doc.error = None
            log.info(
            ...
            log.warning(
                "tender %s document %s failed: %r", tender.tender_no, url, exc
            )
            try:
